...
export const Accessories = () => {
  ...
  useEffect(() => {
    const getAllAcc = async () => {
      const data = await fetchCategories("Accessories");
      setListings(data);
    };
    getAllAcc();
  }, []);

  const AllAccessories = listings?.map(
    ({ id, category, name, isHot, price, image, description }) => {
      return (
        <div className="AccessoriesListings" key={id}>
  ...
  return (
    <div>
      ...
      <div className="AllListings">{AllAccessories}</div>
    </div>
  );
};
