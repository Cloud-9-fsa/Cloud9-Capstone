...
export const Outdoor = () => {
  ...
  useEffect(() => {
    const getAllOutdoor = async () => {
      const data = await fetchCategories("Outdoor");
      setListings(data);
    };
    getAllOutdoor();
  }, []);

  const AllOutdoor = listings?.map(
    ({ id, category, name, isHot, price, image, description }) => {
      return (
        <div className="OutdoorListings" key={id}>
  ...
  return (
    <div>
      ...
      <div className="AllListings">{AllOutdoor}</div>
    </div>
  );
};
